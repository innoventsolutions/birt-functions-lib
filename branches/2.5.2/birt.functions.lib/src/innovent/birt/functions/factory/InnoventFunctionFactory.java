...
public class InnoventFunctionFactory implements IScriptFunctionFactory {
	...
	/**
	 * Factory method to build a class that matches the function name. If you want
	 * to have a function that does not match you can look for other function names
	 * and instantiate the class as needed. Best to just name your Function classes
	 * like the function name.
	 * 
	 */
	public IScriptFunctionExecutor getFunctionExecutor(String functionName) throws BirtException {
		/*
		 * example of non-conforming class name Not required if FunctionName defined
		 * plugin.xml matches the class name
		 * 
		 * if (ModifySurveySql.class.getSimpleName().equals(functionName)){ return new
		 * ModifySurveySql(); }
		 */

		String fullClassName = package_name + "." + functionName;
		try {
			Class<? extends IScriptFunctionExecutor> functionClass = Class.forName(fullClassName)
