...
public class DecimalMedianTest {
	private Accumulator prepare() {
		final IAggrFunction fn = new DecimalMedian();
	...
	public void testAccumulateIntegers() {
		...
		try {
			...
			accumulator.finish();
			final var value = accumulator.getValue();
			final var expected = BigDecimal.valueOf(2);
			Assert.assertEquals(expected, value);
		}
		catch (final DataException e) {
			Assert.fail(e.toString());
	...
	public void testAccumulateDoubles() {
		...
		try {
			...
			accumulator.finish();
			final var value = accumulator.getValue();
			final var expected = BigDecimal.valueOf(2.0);
			Assert.assertEquals(expected, value);
		}
		catch (final DataException e) {
			Assert.fail(e.toString());
