...
public class DecimalNpvTest {
	private Accumulator prepare() {
		final IAggrFunction fn = new DecimalNpv();
	...
	public void testAccumulateBigDecimalZero() {
		...
		try {
			...
			final var expected = new BigDecimal("0E+1");
	...
	public void testAccumulateBigDecimal() {
		...
		try {
			accumulator.start();
			accumulator.onRow(new Object[] { new BigDecimal(1), Double.valueOf(4) });
			...
			accumulator.finish();
			final var value = accumulator.getValue();
			final var expected = new BigDecimal("0.26176");
			Assert.assertEquals(expected, value);
		}
		catch (final DataException e) {
			Assert.fail(e.toString());
	...
	public void testAccumulateIntegers() {
		...
		try {
			...
			accumulator.finish();
			final var value = accumulator.getValue();
			final var expected = new BigDecimal(1.28125);
			Assert.assertEquals(expected, value);
		}
		catch (final DataException e) {
			Assert.fail(e.toString());
